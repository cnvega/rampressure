...
/**
  ...
  - VERBOSE: Enables notification while loading files and allocating memory.
  - PARTICLE_SKIP_VEL: Doesn't load velocities.
 */

/**
  \brief Header-type class. 
 */
class Header {
  public:
	  int      npart[6];
  ...
  /**
    ...
    \param pnum particle type: 0..5
    \param fnum number of file: 0..N
    \return First index for finding particles of \e pnum in \e fnum.
   */
  int calculate_offset(Header*, int, int);
  ...
  /**
    ...
    \return First index for finding particles of \e pnum in \e fnum.
   */
  int calculate_offset_withmasses(Header*, int, int);

};

/**
  \brief Structure to storage particle positions and velocities.
 */
struct Particle {
	float  Pos[3];
...
/**
  ...
  \param dst Pointer to the destination particle.
  \param src Pointer to the source data particle.
 */
void copyParticle(Particle*, Particle*);


/**
  \brief Class for loading a snapshot.
 */
...
class Snapshot {
  ...
  /**
    ...
    \param *fname String containing the path and mean name of the snapshot file.
    \param nfiles Number of files in with the snapshot is divided.
    \return Pointer to a new snapshot containing all the data loaded.
   */
    void loadSnapshot(char*, int);
};
