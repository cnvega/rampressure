...
{
	int offset1, offset2, kc, ic;
	offset1 = 0;
	for(kc=0; kc<k; kc++)
		offset1 += headers[0].npartTotal[kc];
	
	offset2 = 0;
	for(ic=0; ic<i; ic++)
		offset2 += headers[ic].npart[k];
...
/* Fragment a header into fraNum fragments */
HEADER * fragmentHeader(HEADER *header, int fracNum)
{
	HEADER *newheader;
	int i, k, n;

	newheader = malloc(fracNum*sizeof(HEADER));
	for(i=0; i<fracNum; i++)
	{
		newheader[i] = header[0];
		for(k=0; k<6; k++)
			newheader[i].npart[k] = 0;
			newheader[i].num_files = fracNum;
	}

	for(k=0; k<6; k++)
		for(i=0, n=0; n<header[0].npartTotal[k]; n++, i++)
		{
			if(i == fracNum)
				i = 0;
			newheader[i].npart[k]++;
...
/* Set position and velocity in a partcile */
...
void copyParticle(PARTICLE *destiny, PARTICLE *source)
{
	...
	for(i=0; i<3; i++)
	{
		destiny->Pos[i] = source->Pos[i];
		#ifndef ST_SKIP_VEL
		destiny->Vel[i] = source->Vel[i];
		#endif
	}
}
...
void freeSnapshot(SNAPSHOT *snapshot)
{
	...
	free(snapshot->particles);
	free(snapshot->IDs);
	free(snapshot->Mass);
	free(snapshot->Energy);
	free(snapshot->Density);
	free(snapshot->HSML);
	free(snapshot);
}
...
SNAPSHOT * loadSnapshot(char *filename, int files)
{
	...
	snapshot = createSnapshot();
	snapshot->numFiles = files;

	hds = malloc(files*sizeof(HEADER));
